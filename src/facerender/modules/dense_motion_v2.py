"""
Change model input from dicts to all tensors
"""
...
class DenseMotionNetwork(nn.Module):
    ...
    def create_sparse_motions(self, feature, kp_driving, kp_driving_jacobian, kp_source, kp_source_jacobian):
        ...
        if kp_driving_jacobian is not None and kp_source_jacobian is not None:
    ...
    def forward(self, feature, kp_driving, kp_source, kp_driving_jacobian = None, kp_source_jacobian = None):
        bs, _, d, h, w = feature.shape

        feature = self.compress(feature)
        feature = self.norm(feature)
        feature = F.relu(feature)
        # torch.save(feature.detach().cpu(), './tensorrt/dense_motion/partial/feature.pt')
        out_dict = dict()
        sparse_motion = self.create_sparse_motions(feature, kp_driving, kp_driving_jacobian, kp_source, kp_source_jacobian)
        # torch.save(sparse_motion.detach().cpu(), './tensorrt/dense_motion/partial/output.pt')
        ...
        # torch.save(input_.detach().cpu(), './tensorrt/dense_motion/hourglass/input.pt')
        prediction = self.hourglass(input_)
        # torch.save(prediction.detach().cpu(), './tensorrt/dense_motion/hourglass/output.pt')

        # torch.save(prediction.detach().cpu(), './tensorrt/dense_motion/mask/input.pt')
        mask = self.mask(prediction)
        # torch.save(mask.detach().cpu(), './tensorrt/dense_motion/mask/mask.pt')
